...
func (p hashPrefixes) Validate() error {
	...
	for _, h := range p {
		switch {
		case !h.IsValid():
			return errors.New("webrisk: invalid hash")
		case hp >= h:
			return errors.New("webrisk: unsorted hash list")
		case h.HasPrefix(hp) && hp != "":
			return errors.New("webrisk: non-unique hash prefix")
		}
		hp = h
	}
	return nil
	...
	if input.RawHashes != nil {
		for _, raw := range input.GetRawHashes() {
			...
			if raw.PrefixSize < minHashPrefixLength || raw.PrefixSize > maxHashPrefixLength {
				return nil, errors.New("webrisk: invalid hash prefix length")
			}
			if len(raw.RawHashes)%int(raw.PrefixSize) != 0 {
				return nil, errors.New("webrisk: invalid raw hashes")
			}
			...
			output = append(output, hashes...)
		}
	}

	if input.RiceHashes != nil {
...
func decodeIndices(input *pb.ThreatEntryRemovals) ([]int32, error) {
	var ind []int32
	if input.RawIndices != nil {
		raw := input.GetRawIndices()
		if raw == nil {
			return nil, errors.New("webrisk: invalid raw indices")
		}
		ind = append(ind, raw.Indices...)
	}
	if input.RiceIndices != nil {
...
func decodeRiceIntegers(rice *pb.RiceDeltaEncoding) ([]uint32, error) {
	...
	if br.BitsRemaining() >= 8 {
		return nil, errors.New("webrisk: unconsumed rice encoded data")
	}
	return values, nil
}

// riceDecoder implements Golomb-Rice decoding for the Web Risk API.
//
